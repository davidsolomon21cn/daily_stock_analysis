...
| `model_access_denied` | 模型被禁用、未开通、账号不可见或当前 key 无权限访问。 | 先查看测试结果里的“本次测试模型”，在服务商控制台确认该模型已开通；必要时调整模型顺序、移除不可用模型，或点击「获取模型」核对账号可见模型。 |
...
### `model_access_denied` 兼容语料与审计依据

- 本仓库将返回 `model_access_denied` 的场景归为模型可见性/开通状态类失败，依据是后端服务内的关键报错片段匹配。
- 可审计样本与回归入口见：`tests/fixtures/llm_error_classification.json`（与 `tests/test_system_config_service.py` 中的 `test_test_llm_channel_classifies_model_access_denied_cases_from_fixture` 同步）。
- 外部兼容语义依据仍按仓库既有约束执行：`litellm>=1.80.10,!=1.82.7,!=1.82.8,<2.0.0`，并结合 LiteLLM 与 OpenAI Chat Completions 兼容行为进行回归测试。
- 如果你在实际服务商返回其他新文案（如模型停用/未开通/可见性限制），请在本地补充 fixture 后提交回归用例，否则将按 `unknown_error` 兜底处理并逐步跟进。
