...
> When a third-party relay / aggregator returns `OpenAIException - Your request was blocked`, the service only treats it as upstream auth/permission for the exact text `Your request was blocked`; generic `content policy` / `risk control` wording is not forced into this branch and should be triaged via normal auth/network flow. Confirm the Base URL is the provider's OpenAI-compatible endpoint (usually ending in `/v1`, without manually appending `/chat/completions`), the model name exactly matches the provider console, and `ANSPIRE_API_KEYS`, `AIHUBMIX_KEY`, `GEMINI_API_KEY` are not mixed with another relay's Base URL / model.
