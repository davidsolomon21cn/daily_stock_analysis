...
- OpenAI Error Handling / status and error codes: <https://platform.openai.com/docs/guides/error-codes>
...
| **Connection test returns `OpenAIException - Your request was blocked`** | The service classifies this as upstream auth/permission only when the exact text `Your request was blocked` appears; generic `content policy` / `risk control` messages are intentionally left to the generic auth/network branch for safer triage. | 1. Confirm the Base URL is the provider's OpenAI-compatible endpoint, usually ending in `/v1`, without manually appending `/chat/completions`.<br> 2. Match the model name exactly with the provider console and confirm the current Key can access that model.<br> 3. Do not mix `ANSPIRE_API_KEYS`, `AIHUBMIX_KEY`, `GEMINI_API_KEY`, and another relay's Base URL / model. |
