...
- OpenAI Error Handling / status/error code：<https://platform.openai.com/docs/guides/error-codes>
...
| **测试连接报 `OpenAIException - Your request was blocked`** | 系统仅对报错文案中的 `Your request was blocked` 精确匹配进行上游权限类归类；`content policy` / `risk control` 等文本目前按通用网络/鉴权分支处理。 | 1. 确认 Base URL 是代理商提供的 OpenAI-compatible endpoint，通常需要 `/v1`，不要额外拼 `/chat/completions`。<br> 2. 模型名必须与代理商控制台完全一致，且当前 Key 已开通该模型。<br> 3. 不要混用 `ANSPIRE_API_KEYS`、`AIHUBMIX_KEY`、`GEMINI_API_KEY` 和其他代理商的 Base URL / 模型。 |
