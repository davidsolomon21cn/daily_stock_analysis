...
| **测试连接报 `OpenAIException - Your request was blocked`** | 上游 OpenAI-compatible 代理商、风控或账号权限拦截了请求，不是本地网络断开。 | 1. 确认 Base URL 是代理商提供的 OpenAI-compatible endpoint，通常需要 `/v1`，不要额外拼 `/chat/completions`。<br> 2. 模型名必须与代理商控制台完全一致，且当前 Key 已开通该模型。<br> 3. 不要混用 `ANSPIRE_API_KEYS`、`AIHUBMIX_KEY`、`GEMINI_API_KEY` 和其他代理商的 Base URL / 模型。 |
